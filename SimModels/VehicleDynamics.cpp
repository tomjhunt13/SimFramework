...
namespace Models {
    ...
    void VehicleDynamics::SetParameters(float initialPosition, float initialVelocity, float mass, float Cd, float A, float rho, float peakBrakeForce, float rollingResistance) {
        ...
        Eigen::Matrix<float, 2, 5> B;
        B << 0, 0, 0, 0, 0, 1.f / mass, -1.f / mass, -1.f / mass, -1.f / mass, -1.f / mass;

        Eigen::Matrix<float, 2, 2> C;
        C << 1.f, 0.f, 0.f, 1.f;

        Eigen::Matrix<float, 2, 5> D;
        D << 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f;
            ...
            const SimFramework::Signal<float>* inBrakePedal)
    {
        ...
        this->m_Vectorise.Configure({inTyreForce, this->m_AeroDrag.OutForce(), this->m_Gravity.OutForce(), this->m_Brake.OutForce(), this->m_RollingResistance.OutForce()});
    ...
    SimFramework::BlockList VehicleDynamics::Blocks()
    {
        return {{&(this->m_ConstWeight)},
                {&(this->m_StateSpace)},
                {&(this->m_AeroDrag), &(this->m_Gravity), &(this->m_Brake), &(this->m_Vectorise), &(this->m_Mask), &(this->m_RollingResistance)},
                {},
                {}};
    };
